...
export const matchSellers = (requirement) => {
    const listings = store.getListings().filter(l => l.status === 'active' &&
    ...
    const matches = listings.map(listing => {
        ...
        // distanceScore (0.15)
        const distance = getDistance(requirement.deliveryLocation.lat, requirement.deliveryLocation.lng, listing.location.lat, listing.location.lng);
        if (distance <= 50) {
            score += 0.15;
