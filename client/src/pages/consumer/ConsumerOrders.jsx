...
export const ConsumerOrders = () => {
    ...
    return (<div className="max-w-4xl mx-auto space-y-6">
      ...
      {orders.length === 0 ? (<div className="text-center py-20 bg-white rounded-xl border border-gray-200">
          ...
          {orders.map(order => (<Card key={order.id} className="overflow-hidden hover:border-gray-300 transition-colors">
              ...
              <div className="p-6 flex flex-col md:flex-row justify-between items-start md:items-center">
                <div className="mb-4 md:mb-0">
                  <div className="mb-2">
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold uppercase tracking-wider ${order.status === 'delivered' ? 'bg-green-100 text-green-800' :
                  ...
                  <div className="text-sm text-gray-900 font-medium space-y-1">
                    {order.items.map((item, i) => (<p key={i}>{item.name} x {item.quantity} {item.unit}</p>))}
                  </div>
                </div>
                
                <div className="flex space-x-3 w-full md:w-auto">
                  {order.status === 'delivered' ? (<Button variant="outline" className="w-full md:w-auto">Rate Seller</Button>) : (<Button className="w-full md:w-auto" rightIcon={<ChevronRight className="w-4 h-4"/>} onClick={() => window.location.href = `/consumer/tracking/${order.id}`}>
                      Track Order
                    </Button>)}
