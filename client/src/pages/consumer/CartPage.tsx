              ...
              <CardBody className="p-4 sm:p-6 flex flex-col sm:flex-row items-start sm:items-center">
                <div className="w-full sm:w-24 h-24 bg-gray-100 rounded-lg overflow-hidden flex-shrink-0 mb-4 sm:mb-0 mr-0 sm:mr-6">
                  {item.listing.images && item.listing.images[0] ? (
                    <img src={item.listing.images[0]} alt={item.listing.productName} className="w-full h-full object-cover" />
                  ) : (
                ...
                <div className="flex-1 w-full">
                  ...
                  <div className="flex justify-between items-end mt-4">
                    <div className="flex items-center space-x-3 bg-gray-50 rounded-lg p-1 border border-gray-200">
                      <button 
                        onClick={() => updateQuantity(item.listingId, Math.max(item.listing.minOrderQuantity, item.quantity - 1))}
                        disabled={item.quantity <= item.listing.minOrderQuantity}
                      ...
                      <button 
                        onClick={() => updateQuantity(item.listingId, Math.min(item.listing.availableQuantity, item.quantity + 1))}
