        ...
        <CardContent className="p-4 border-b border-gray-200">
          <div className="flex flex-col sm:flex-row gap-4 justify-between">
            <div className="relative w-full max-w-md">
              ...
              <Input value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-10" placeholder="Search orders..."/>
            </div>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {orders.map((order) => (<tr key={order.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    {order.status === 'In Transit' && (<div className="text-xs text-gray-500 mt-1 flex items-center">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    {order.status === 'In Transit' ? (<Button variant="outline" size="sm" className="w-full text-xs">Track</Button>) : order.status === 'Delivered' ? (<Button variant="outline" size="sm" className="w-full text-xs text-yellow-600 border-yellow-200 hover:bg-yellow-50">
                        <Star className="mr-1 h-3 w-3"/> Rate
                      </Button>) : (<Button variant="ghost" size="sm" className="w-full text-xs">Details</Button>)}
                  </td>
                </tr>))}
            </tbody>
          </table>
