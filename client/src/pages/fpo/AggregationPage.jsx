...
import { Plus, TrendingUp, Users, Save, Share } from 'lucide-react';
...
export default function AggregationPage() {
    ...
    const CreateAggregationForm = () => (<div className="space-y-6">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
            <div className="space-y-3">
              {contributions.map((c, i) => (<div key={i} className="flex justify-between items-center bg-white p-2 rounded shadow-sm">
    ...
    return (<div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        {!isCreating && (<Button onClick={() => setIsCreating(true)}>
      ...
      {isCreating ? (<Card>
          ...
          <div className="flex space-x-2 border-b border-gray-200">
            {['Active', 'Completed'].map(status => (<button key={status} onClick={() => setActiveTab(status.toLowerCase())} className={`px-4 py-2 border-b-2 text-sm font-medium ${activeTab === status.toLowerCase()
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {aggregations
                ...
                .map(agg => (<Card key={agg.id} className="overflow-hidden hover:shadow-md transition-shadow flex flex-col">
                <CardHeader className="pb-2 border-b">
                  <div className="flex justify-between items-start">
                    ...
                    <Badge variant={agg.status === 'Listed' ? 'success' :
                ...
                <CardContent className="pt-4 flex-grow">
                  <div className="mb-4">
                    ...
                    <div className="w-full bg-gray-200 rounded-full h-2.5">
                      <div className={`h-2.5 rounded-full ${agg.current >= agg.target ? 'bg-green-500' : 'bg-primary-500'}`} style={{ width: `${Math.min((agg.current / agg.target) * 100, 100)}%` }}></div>
                    </div>
                  </div>
                ...
                <CardFooter className="bg-gray-50 border-t flex gap-2">
                  ...
                  {agg.status === 'In Progress' && (<Button className="flex-1 text-sm h-9">Publish</Button>)}
                </CardFooter>
              </Card>))}
          </div>
        </>)}
