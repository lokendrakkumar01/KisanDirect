...
export default function UserManagement() {
    ...
    return (<div className="space-y-6">
      ...
      <div className="flex gap-4 border-b">
        {['All', 'Farmers', 'FPOs', 'Consumers', 'Bulk Buyers'].map(tab => (<button key={tab} onClick={() => setActiveTab(tab)} className={`pb-2 px-1 text-sm font-medium ${activeTab === tab ? 'border-b-2 border-green-600 text-green-600' : 'text-gray-500 hover:text-gray-700'}`}>
            {tab}
          </button>))}
      </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y">
                {mockUsers.map(u => (<tr key={u.id} className="hover:bg-gray-50">
                    <td className="p-4">
                    ...
                    <td className="p-4">
                      <Badge variant={u.verification === 'Verified' ? 'success' :
                u.verification === 'Pending' ? 'warning' : 'destructive'}>{u.verification}</Badge>
                    </td>
