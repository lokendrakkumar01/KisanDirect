...
export const StatusTimeline = ({ currentStatus, history = [] }) => {
    if (currentStatus === 'cancelled') {
        return (<div className="p-4 bg-red-50 text-red-700 rounded-lg border border-red-100">
        This order has been cancelled.
      </div>);
    }
    const currentIndex = statusOrder.indexOf(currentStatus);
    return (<div className="relative pl-4 border-l-2 border-gray-200 space-y-6">
      {statusOrder.map((status, index) => {
            const isCompleted = index <= currentIndex;
            const isCurrent = index === currentIndex;
            const historyItem = history.find(h => h.status === status);
            return (<div key={status} className="relative">
            <div className={`absolute -left-[23px] p-1 rounded-full ${isCompleted ? 'bg-green-500 text-white' : 'bg-gray-200 text-gray-400'}`}>
