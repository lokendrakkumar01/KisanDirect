...
export const RouteMap: React.FC<RouteMapProps> = ({ stops, className = 'h-96' }) => {
  if (stops.length === 0) return null;

  const positions: [number, number][] = stops.map(stop => [stop.point.lat, stop.point.lng]);
    ...
    <div className={`w-full rounded-xl overflow-hidden border border-gray-200 z-0 ${className}`}>
      <MapContainer center={center} zoom={8} style={{ height: '100%', width: '100%' }}>
        ...
        {stops.map((stop, index) => (
          <Marker 
            key={`${stop.point.id}-${index}`} 
            position={[stop.point.lat, stop.point.lng]}
            icon={getIconForType(stop.point.type)}
          >
            <Popup>
              <div>
                ...
                {stop.point.quantity > 0 && (
                  <p className="text-xs font-medium mt-1">Load: {stop.point.quantity} Qtl</p>
                )}
              </div>
            </Popup>
          </Marker>
